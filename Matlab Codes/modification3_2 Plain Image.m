...
clear all
close all
RGB_IMAGE=imread('4.1.07.tiff'); %Loading of the color image  
RGB_IMAGE=imresize(RGB_IMAGE,[256 256]);
figure(1)
imshow(RGB_IMAGE);
title('RGB image');
GRAY_IMAGE=rgb2gray(RGB_IMAGE); %Conversion of the color image to Gray Scale image 
figure(2)
imshow(GRAY_IMAGE);
title('GRAY image');
...
for i=1:1:row
    for j=1:1:col
      finalOutput1{i,j} = dec2bin(GRAY_IMAGE(i,j),8);
    end
end 
...
uaci=uacifun(inverse_im,GRAY_IMAGE)
npcr1=npcrfun(inverse_im,GRAY_IMAGE,256,256)
corrc(inverse_im,GRAY_IMAGE)
%PSNR Peak Signal Noise Ratio
sum=0;
for i=1:row
    for j=1:col
        k=(double(GRAY_IMAGE(i,j))-double(inverse_im(i,j)));
        sum=sum+k*k;
    end
end
MSE=((sum/(256*256)));
